...
public class Input {
  ...
  public static List<Point> parse(String input) {
    return Stream.of(input.split("\n")).map(Input::createPoint).collect(Collectors.toList());
  }

  public static Point createPoint(String line) {
    String[] values = line.split(", ");
    Position p = parsePosition(values[0]);
    Velocity v = parseVelosity(values[1]);
    Acceleration a = parseAcceleration(values[2]);
    return new Point(p, v, a);
  }

  private static Position parsePosition(String value) {
    ParseTriple parseTriple = new ParseTriple(value);
    return new Position(parseTriple.getX(), parseTriple.getY(), parseTriple.getZ());
  }

  private static Velocity parseVelosity(String value) {
    ParseTriple parseTriple = new ParseTriple(value);
    return new Velocity(parseTriple.getX(), parseTriple.getY(), parseTriple.getZ());
  }

  private static Acceleration parseAcceleration(String value) {
    ParseTriple parseTriple = new ParseTriple(value);
    return new Acceleration(parseTriple.getX(), parseTriple.getY(), parseTriple.getZ());
  }
}
